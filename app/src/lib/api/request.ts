...
import { createApiRequestError, getNetworkErrorMessage } from "@/lib/api/error";
...
const getProblemDetails = (value: unknown): ProblemDetailsDto | undefined => (isProblemDetails(value) ? value : undefined);

const errorMiddleware: Middleware = {
  onError: async ({ error }: MiddlewareCallbackParams & {
    error: unknown
