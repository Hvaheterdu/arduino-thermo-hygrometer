...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  protected ResponseEntity<@NonNull Object> handleMethodArgumentNotValid(
    ...
    log.error(
        "Method argument not valid exception with message={}",
        methodArgumentNotValidException.getMessage());

    List<ProblemDetailsValidationErrorDto> errors =
        methodArgumentNotValidException.getBindingResult().getFieldErrors().stream()
            .map(this::toValidationError)
            .toList();
    String traceId = MDC.get("traceId");
    var body =
        ProblemDetailsDto.builder()
  ...
  public ResponseEntity<ProblemDetailsDto> handleResourceNotFound(
    ...
    log.error(
        "Resource not found exception with message={}", resourceNotFoundException.getMessage());
  ...
  public ResponseEntity<ProblemDetailsDto> handleResourceNotCreated(
      final ResourceNotCreatedException resourceNotCreatedException,
      final HttpServletRequest request) {
    log.error(
        "Resource not created exception with message={}", resourceNotCreatedException.getMessage());
    ProblemDetailsDto body =
        buildProblemDetail(
            HttpStatus.BAD_REQUEST,
            "resource-not-created",
            "Resource not created.",
            resourceNotCreatedException.getMessage(),
            request);
