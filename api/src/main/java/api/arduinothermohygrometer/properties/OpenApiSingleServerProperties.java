...
public record OpenApiSingleServerProperties(
  ...
  public Server generateOpenApiServer() {
    ...
    variables.forEach(
        (key, openApiServerVariableProperties) ->
            serverVariables.addServerVariable(
                key,
                new ServerVariable()._default(openApiServerVariableProperties.defaultValue())));

    return new Server().url(url).description(description).variables(serverVariables);
  }
}
